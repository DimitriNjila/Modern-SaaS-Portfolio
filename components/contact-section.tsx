  ...
  const processCommand = (command: string) => {
    ...
    if (step === "message") {
      ...
      setTimeout(() => {
        ...
        addLine("Thank you for reaching out. I'll get back to you soon!", "info")
        addLine("", "prompt")
        addLine('Type "contact" to send another message or "help" for commands', "info")
        addLine("", "prompt")
        setStep("idle")
    ...
    switch (cmd) {
      ...
      case "about":
        addLine("Full-stack developer passionate about creating elegant solutions.", "info")
        addLine("Specialized in React, Next.js, and modern web technologies.", "info")
        break
    ...
    <section id="contact" className="py-32 px-6 bg-secondary/30">
      <div className="container mx-auto max-w-5xl">
        <div className="space-y-4 mb-16 text-center">
          <h2 className="text-4xl md:text-5xl font-bold text-balance">Get In Touch</h2>
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto text-pretty leading-relaxed">
            Interested in working together? Use the interactive terminal below to send me a message. Type "contact" to
            get started or "help" to see available commands.
          </p>
        </div>
          ...
          >
            {lines.map((line, index) => (
              <div key={index} className="leading-relaxed">
                {line.type === "prompt" && <span className="text-primary select-none">{"> "}</span>}
                {line.type === "input" && <span className="text-foreground">{line.text}</span>}
